...
<!--          <a @click='handleEdit(record)'>编辑</a>-->
          <a v-has="'SalTicket:delivery'" @click='handleDeliveryUpdate(record)' v-if='record.status === 2'>修改物流信息</a>
          <a-divider v-has="'SalTicket:delivery'" v-if='record.status === 2' type='vertical' />

          <a-popconfirm v-has="'SalTicket:delivery'" v-if='record.status === 2' title='确定确认收货吗?' @confirm='() => handleConfirmReceipted(record.id)'>
            <a>确认收货</a>
          </a-popconfirm>
          <a-divider v-has="'SalTicket:delivery'" v-if='record.status === 2' type='vertical' />
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/sale/salTicket/list/2',
        delete: '/sale/salTicket/delete',
        deleteBatch: '/sale/salTicket/deleteBatch',
        exportXlsUrl: 'sale/salTicket/exportXls/2',
        importExcelUrl: 'sale/salTicket/importExcel',
        confirmReceiptedUrl: 'sale/salTicket/confirmReceipted'
      }
    }
  },
  computed: {
  ...
  methods: {
    handleConfirmReceipted(id) {
      ...
      postAction(`${this.url.confirmReceiptedUrl}`, formData).then(res => {
        if (res.success) {
